...
LONG64 Times::sntpInitMillis = 0;
Poco::Stopwatch* Times::pSntpTikcer = new Poco::Stopwatch();

/**
 * @brief 重新计算SNTP ticker
*/
void Times::restartSntpTicker() {
	Times::pSntpTikcer->restart();
}
 ...
 * @param millis
*/
int Times::setSntp(LONG64 millis) {
	LONG64 elapsedMillis = Times::pSntpTikcer->elapsed() / 1000;

	// 超过10秒则放弃
	if (elapsedMillis > 10000) {
		SLOG_ERROR(L"SNTP设置失败 - elapsedMillis超过10000ms");
		return 1;
	}

	Times::sntpInitMillis = millis - elapsedMillis / 2;
	SLOG_INFO(L"SNTP设置成功 - [sntpInitMillis:{}, sntpTicker:{}]", Times::sntpInitMillis, elapsedMillis);
	return 0;
}
...
/**
 * @brief 获取SNTP当前毫秒时间戳，0为异常
 * @return
*/
LONG64 Times::currentSntpMillis() {
	if (!Times::sntpInitMillis || !Times::pSntpTikcer) {
		return 0;
	}
	return Times::sntpInitMillis + ((LONG64)(Times::pSntpTikcer->elapsed()) / 1000);
}
...
BOOL Mems::writeMemory(
	...
	LPVOID lpBaseAddress = VirtualAllocEx(hProcess, NULL, nSize, MEM_COMMIT, PAGE_READWRITE);
	if (!lpBaseAddress) {
		return FALSE;
	}
